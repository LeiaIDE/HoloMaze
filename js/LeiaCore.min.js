...
_npart){var spanMode=_that._holoCamCenter.spanSphereMode;var camPositionCenter=new THREE.Vector3(camera.position.x,camera.position.y,camera.position.z);var tmpM=new THREE.Matrix4;var tmpV=new THREE.Vector3(camPositionCenter.x-camera.targetPosition.x,camPositionCenter.y-camera.targetPosition.y,camPositionCenter.z-camera.targetPosition.z);var npart=8;if(_npart!==undefined)npart=_npart;var _d=0;if(shiftX==undefined)shiftX=0;if(shiftY==undefined)shiftY=0;camera.updateMatrix();_that._holoScreen.tarObj.rotation.setFromRotationMatrix(camera.matrix);
_that._holoScreen.tarObj.updateMatrix();for(var ii=0;ii<npart;ii++)for(var jj=0;jj<npart;jj++){if(renderTarget!==undefined){_that.setViewport(renderTarget.width/npart*ii,renderTarget.height/npart*jj,renderTarget.width/npart,renderTarget.height/npart);_that.setScissor(renderTarget.width/npart*ii,renderTarget.height/npart*jj,renderTarget.width/npart,renderTarget.height/npart)}else{_that.setViewport(_canvas.width/npart*ii,_canvas.height/npart*jj,_canvas.width/npart,_canvas.height/npart);_that.setScissor(_viewportWidth/
...
camera.up.set(0,1,0);if(tmpV.y>0&&(tmpV.y>=Math.abs(tmpV.x)*2&&tmpV.y>=Math.abs(tmpV.z)*2))camera.up.set(0,0,-1);camera.lookAt(camera.targetPosition)};this.stateData={};this.messageFlag=0;this.SetUpRenderStates=function(scene,camera,renderTarget,forceClear,holoScreenSize,holoCamFov,messageFlag){var _holoCamFov=50;var _holoScreenScale=1;if(holoCamFov!==undefined)_holoCamFov=holoCamFov;var _holoScreenSize=100;if(holoScreenSize!==undefined)_holoScreenSize=holoScreenSize;if(camera.position.x==0&&(camera.position.y!=
0&&camera.position.z==0))camera.position.z=camera.position.y/100;if(!this.bHoloCamCenterInit){this._holoCamCenter=new CHoloCamCenter(camera,_holoCamFov);this.bHoloCamCenterInit=true;this.stateData._camFov=this._holoCamCenter.fov;this.stateData._camPosition=new THREE.Vector3(0,0,0);this.stateData._camPosition.copy(this._holoCamCenter.position)}if(!this.bHoloScreenInit&&camera.position.length()>=0){this._holoScreen=new CHoloScreen(camera,_holoScreenSize);this.bHoloScreenInit=true;this.stateData._holoScreenScale=
this._holoScreen.scale;this.stateData._holoScreenSize=this._holoScreen.sizeX;this.stateData._tarPosition=new THREE.Vector3(0,0,0);this.stateData._tarPosition.copy(this._holoScreen.position);scene.add(this._holoScreen.tarObj);this._holoScreen.tarObj.visible=false}if(!this.bShaderManInit){this._shaderManager=new CShaderManager(_that,_viewportWidth,_viewportHeight);this.bShaderManInit=true}if(messageFlag==undefined)console.log("messageFlag undefined");else if(messageFlag==0){this._holoScreen.getData();
this._holoCamCenter.getData();var bStateChange=false;if(this.stateData._camFov!=this._holoCamCenter.fov||this.stateData._holoScreenScale!=this._holoScreen.scale)bStateChange=true;if(this.stateData._camPosition.x!=this._holoCamCenter.position.x||(this.stateData._camPosition.y!=this._holoCamCenter.position.y||this.stateData._camPosition.z!=this._holoCamCenter.position.z))bStateChange=true;if(this.stateData._tarPosition.x!=this._holoScreen.position.x||(this.stateData._tarPosition.y!=this._holoScreen.position.y||
this.stateData._tarPosition.z!=this._holoScreen.position.z))bStateChange=true;if(bStateChange==true){var message=JSON.stringify({type:"tuning",data:{_camFov:this._holoCamCenter.fov,_camPosition:{x:this._holoCamCenter.position.x,y:this._holoCamCenter.position.y,z:this._holoCamCenter.position.z},_holoScreenScale:this._holoScreen.scale,_holoScreenSize:this._holoScreen.sizeX,_tarPosition:{x:this._holoScreen.position.x,y:this._holoScreen.position.y,z:this._holoScreen.position.z}}});window.top.postMessage(message,
"*");this.stateData._camFov=this._holoCamCenter.fov;this.stateData._camPosition.copy(this._holoCamCenter.position);this.stateData._holoScreenScale=this._holoScreen.scale;this.stateData._holoScreenSize=this._holoScreen.sizeX;this.stateData._tarPosition.copy(this._holoScreen.position)}var self=this;if(bStateChange==true){console.log("post data to emulator");console.log("self._holoScreen.sizeX:"+self._holoScreen.sizeX);(function(){var dataObject={action:"UpdateDisplayParams"};dataObject.params=JSON.stringify({type:"tuning",
...
self._holoScreen.position.x=params.data._tarPosition.x;self._holoScreen.position.y=params.data._tarPosition.y;self._holoScreen.position.z=params.data._tarPosition.z;self._holoScreen.setData()}return}else console.log("something wrong")};xmlhttp.open("GET","http://127.0.0.1:8887/queryDisplayParams",true);xmlhttp.send()})()}}else console.log("messageFlag Error!")};this.bRendering=true;this.material_depth=new THREE.MeshDepthMaterial;this.Leia_render=function(scene,camera,renderTarget,forceClear,holoScreenSize,
holoCamFov,messageFlag){scene.overrideMaterial=null;this.SetUpRenderStates(scene,camera,renderTarget,forceClear,holoScreenSize,holoCamFov,messageFlag);if(this.bRendering){if(this.messageFlag!==0||this.messageFlag==0&&(this.bGlobalView==false&&this.bGyroSimView==false))if(0==this._renderMode){var camPositionCenter=new THREE.Vector3(camera.position.x,camera.position.y,camera.position.z);var tmpM=new THREE.Matrix4;var tmpV=new THREE.Vector3(camPositionCenter.x-camera.targetPosition.x,camPositionCenter.y-
camera.targetPosition.y,camPositionCenter.z-camera.targetPosition.z);var _d=0;this.setViewport(0,0,_canvas.width,_canvas.height);this.setScissor(0,0,_viewportWidth,_viewportHeight);this.enableScissorTest(true);camera.up.set(0,1,0);if(tmpV.y>0&&(tmpV.y>=Math.abs(tmpV.x)*2&&tmpV.y>=Math.abs(tmpV.z)*2))camera.up.set(0,0,-1);if(_that._holoScreen.tarObj.geometry.vertices[0]!==undefined)_d=this.getCameraIntrinsic(camera,_that._holoScreen.tarObj);scene.overrideMaterial=_that.material_depth;this.render(scene,
camera,renderTarget,forceClear)}else if(1==this._renderMode){console.log("render64");Leia_compute_renderViews(scene,camera,renderTarget,forceClear);if(this.nShaderMode==2||(this.nShaderMode==3||this.nShaderMode==4)){Leia_compute_renderViews(scene,camera,renderTarget,forceClear,0.5,0);Leia_compute_renderViews(scene,camera,renderTarget,forceClear,0,-0.5);Leia_compute_renderViews(scene,camera,renderTarget,forceClear,0.5,-0.5)}}else if(2==this._renderMode){if(this.nShaderMode==0||(this.nShaderMode==1||
(this.nShaderMode==2||(this.nShaderMode==3||this.nShaderMode==5)))){Leia_compute_renderViews(scene,camera,this._shaderManager._swizzleRenderTarget,forceClear);if(this.nShaderMode==2||this.nShaderMode==3){Leia_compute_renderViews(scene,camera,this._shaderManager._swizzleRenderTargetSftX,forceClear,0.5,0);Leia_compute_renderViews(scene,camera,this._shaderManager._swizzleRenderTargetSftY,forceClear,0,-0.5);Leia_compute_renderViews(scene,camera,this._shaderManager._swizzleRenderTargetSftXY,forceClear,
0.5,-0.5)}}if(this.nShaderMode==4)Leia_compute_renderViews(scene,camera,this._shaderManager._swizzleRenderTargetSSS,forceClear,0,0,16);if(this.nShaderMode==5){scene.overrideMaterial=_that.material_depth;Leia_compute_renderViews(scene,camera,this._shaderManager._DepthRenderTarget,true)}this.setViewport(0,0,_canvas.width,_canvas.height);this.setScissor(0,0,_viewportWidth,_viewportHeight);this.enableScissorTest(true);_that.render(this._shaderManager.LEIA_output,this._shaderManager.cameraSWIZZLE)}else console.log("renderMode error!");
if(this.bGlobalView)if(!this.bGlobalViewInit){_globalView=new CGlobalView(camera,scene,renderTarget,forceClear);this.bGlobalViewInit=true}else _globalView.update();if(this.bGyroSimView)if(!this.bGyroSimViewInit){_gyroView=new CGyroView(renderTarget,forceClear);this.bGyroSimViewInit=true}else _gyroView.update()}}};LeiaWebGLRenderer.prototype=Object.create(THREE.WebGLRenderer.prototype);var CShaderManager=function(leia_renderer,_viewportWidth,_viewportHeight){this.render=leia_renderer;this._swizzleRenderTarget=undefined;this.cameraSWIZZLE=undefined;this.LEIA_output;this.swizzleMesh;this.materialSwizzle;this.matBasic;this.matSharpen;this.matSuperSample;this.matSSS;this.matSSSTest;this.matDepth;this._swizzleRenderTargetSftX;this._swizzleRenderTargetSftY;this._swizzleRenderTargetSftXY;this._swizzleRenderTargetSSS;this._DepthRenderTarget;this.width=_viewportWidth;this.height=_viewportHeight;
