...
var LeiaWebGLRenderer=function(parameters){parameters=parameters||{};THREE.WebGLRenderer.call(this,parameters);if(parameters.renderMode==undefined){this._renderMode=0;console.log("renderMode undefined!")}else{if(parameters.renderMode<=2){this._renderMode=parameters.renderMode;this.bGlobalView=false;this.bGyroSimView=false}else if(parameters.renderMode==3){this.bGlobalView=true;this.bGyroSimView=false;this._renderMode=0}else{this.bGlobalView=false;this.bGyroSimView=true;this._renderMode=0}console.log("setRenderMode:"+
...
this.getData=function(){this.position.copy(this.tarObj.position);this.scale=this.tarObj.scale.x;this.sizeX=originSize*this.scale;this.tarObj.rotation.setFromRotationMatrix(camera.matrix);var __point=new THREE.Vector3;__point.copy(camera.position.clone().sub(camera.targetPosition));var cur_dis=__point.length();var cur_unit=this.tarObj.scale.x*_length;var cur_disByUint=cur_dis/cur_unit;var cur_scale=auxBox.scale.x;auxBox.scale.z=cur_disByUint/8};this.setData=function(){this.tarObj.position.copy(this.position);
this.tarObj.scale.x=this.scale;this.tarObj.scale.y=this.scale;this.tarObj.scale.z=this.scale;this.sizeX=originSize*this.scale;this.tarObj.rotation.setFromRotationMatrix(camera.matrix)}};this._holoCamCenter=undefined;this.bHoloCamCenterInit=false;var CHoloCamCenter=function(camera,_fov){this.position=new THREE.Vector3;this.position.copy(camera.position);this.fov=_fov;this.spanSphereMode=false;var __point=new THREE.Vector3;__point.copy(camera.position.clone().sub(camera.targetPosition));this.origin_dis=
__point.length();var EyeCenterSize=this.origin_dis/160;var geoEyeCenter=new THREE.SphereGeometry(EyeCenterSize,32,32);var matEyeCenter=new THREE.MeshBasicMaterial({color:16711935,transparent:false});this.eyeCenter=new THREE.Mesh(geoEyeCenter,matEyeCenter);this.eyeCenter.position.set(this.position.x,this.position.y,this.position.z);this.eyeCenter.rotation.setFromRotationMatrix(camera.matrix);this.eyeCenter.name="eyeCenter";this.eyeCenter.visible=true;this.eyeCenter.updateMatrix();this.getData=function(){this.position.copy(this.eyeCenter.position);
this.eyeCenter.rotation.setFromRotationMatrix(camera.matrix)};this.setData=function(){this.eyeCenter.position.copy(this.position)}};this._holoClipFrustum=undefined;this.bHoloClipFrustumInit=false;var CHoloClipFrustum=function(_holoCamCenter,_holoScreen,_near,_far){this.holocamcenter=_holoCamCenter;this.holoscreen=_holoScreen;this.nearPlane=new THREE.Object3D;var _length=this.holoscreen.sizeX/4;var halfX=1*_length*2;var halfY=1*_length*1.5;var origin_nearZ=-1*_near;var vtx=[];vtx.push(new THREE.Vector3(halfX,
halfY,0));vtx.push(new THREE.Vector3(halfX,-halfY,0));vtx.push(new THREE.Vector3(-halfX,-halfY,0));vtx.push(new THREE.Vector3(-halfX,halfY,0));var matLine=new THREE.LineBasicMaterial({color:13311});var lineNear=[];for(var ii=0;ii<4;ii++){var linegeo=new THREE.Geometry;linegeo.vertices.push(vtx[ii],vtx[(ii+1)%4]);lineNear.push(new THREE.Line(linegeo,matLine));this.nearPlane.add(lineNear[ii])}this.nearPlane.name="nearPlane";this.nearPlane.position.z=origin_nearZ;this.holocamcenter.eyeCenter.add(this.nearPlane);
this.farPlane=new THREE.Object3D;var origin_farZ=-1*_far;vtx.push(new THREE.Vector3(halfX,halfY,0));vtx.push(new THREE.Vector3(halfX,-halfY,0));vtx.push(new THREE.Vector3(-halfX,-halfY,0));vtx.push(new THREE.Vector3(-halfX,halfY,0));var lineFar=[];for(var jj=0;jj<4;jj++){var linegeo=new THREE.Geometry;linegeo.vertices.push(vtx[jj+4],vtx[(jj+1)%4+4]);lineFar.push(new THREE.Line(linegeo,matLine));this.farPlane.add(lineFar[jj])}this.farPlane.name="farPlane";this.farPlane.position.z=origin_farZ;this.holocamcenter.eyeCenter.add(this.farPlane);
this.frustum=new THREE.Object3D;var lineFrustum=[];for(var kk=0;kk<4;kk++){var linegeo=new THREE.Geometry;linegeo.vertices.push(this.holocamcenter.eyeCenter.position,this.farPlane.children[kk].geometry.vertices[0]);lineFrustum.push(new THREE.Line(linegeo,matLine));this.frustum.add(lineFrustum[kk])}this.holocamcenter.eyeCenter.add(this.frustum);this.distanceNear=-1*origin_nearZ;this.distanceFar=-1*origin_farZ;this.getData=function(){this.distanceNear=-1*this.nearPlane.position.z;this.distanceFar=-1*
this.farPlane.position.z;var distanceZdp=new THREE.Vector3;distanceZdp.copy(this.holocamcenter.eyeCenter.position.clone().sub(this.holoscreen.tarObj.position));var cur_dis=distanceZdp.length();var _scaleNear=this.holoscreen.scale*Math.abs(this.nearPlane.position.z)/cur_dis;var _scaleFar=this.holoscreen.scale*Math.abs(this.farPlane.position.z)/cur_dis;this.nearPlane.scale.x=_scaleNear;this.nearPlane.scale.y=_scaleNear;this.nearPlane.scale.z=_scaleNear;this.farPlane.scale.x=_scaleFar;this.farPlane.scale.y=
_scaleFar;this.farPlane.scale.z=_scaleFar;for(var ll=0;ll<4;ll++){this.frustum.children[ll].geometry.verticesNeedUpdate=true;this.frustum.children[ll].geometry.vertices[0]=new THREE.Vector3(0,0,0);var oldPsoition=new THREE.Vector3;oldPsoition.copy(this.farPlane.children[ll].geometry.vertices[0]);var matW=new THREE.Matrix4;matW=this.farPlane.matrix;oldPsoition.applyMatrix4(matW);this.frustum.children[ll].geometry.vertices[1]=oldPsoition}};this.setData=function(){this.nearPlane.position.z=-1*this.distanceNear;
this.farPlane.position.z=-1*this.distanceFar}};this.bGlobalViewInit=false;var _globalView;var CGlobalView=function(camera,scene,renderTarget,forceClear){var npart=8;this.camMeshs64=[];this.ObjMesh2=[];this.Gcamera=new THREE.PerspectiveCamera(90,_canvas.width/_canvas.height,0.01,4E4);this.init=function(){var vecCend=new THREE.Vector3(camera.position.x,camera.position.y,camera.position.z);var vecGT=new THREE.Vector3((camera.position.x+camera.targetPosition.x)/2,(camera.position.y+camera.targetPosition.y)/
2,(camera.position.z+camera.targetPosition.z)/2);vecCend.x-=vecGT.x;vecCend.y-=vecGT.y;vecCend.z-=vecGT.z;var vecUp=new THREE.Vector3(0,-vecCend.z,vecCend.y);var lengthVecUp=Math.sqrt(vecUp.x*vecUp.x+vecUp.y*vecUp.y+vecUp.z*vecUp.z);vecUp.x/=lengthVecUp;vecUp.y/=lengthVecUp;vecUp.z/=lengthVecUp;var __length=Math.sqrt(vecCend.x*vecCend.x+vecCend.y*vecCend.y+vecCend.z*vecCend.z);vecUp.x=vecUp.x*2*__length+vecGT.x;vecUp.y=vecUp.y*2*__length+vecGT.y;vecUp.z=vecUp.z*2*__length+vecGT.z;this.Gcamera=new THREE.PerspectiveCamera(90,
_canvas.width/_canvas.height,0.01,4E4);this.Gcamera.position.x=vecUp.x;this.Gcamera.position.y=vecUp.y;this.Gcamera.position.z=vecUp.z;this.Gcamera.up.x=vecCend.x;this.Gcamera.up.y=vecCend.y;this.Gcamera.up.z=vecCend.z;this.Gcamera.lookAt(new THREE.Vector3(vecGT.x,vecGT.y,vecGT.z));this.LocalControls=new dragControls(this.Gcamera);this.LocalControls.screen.left=0;this.LocalControls.screen.top=_canvas.height*(1-_that.GObserveView.bottom-_that.GObserveView.height);this.LocalControls.screen.width=_canvas.width*
...
this.farPlaneControls.screen.height=_canvas.height*_that.GObserveView.height;var camGeometry=new THREE.CylinderGeometry(0,__length/40,__length/20,20);camGeometry.applyMatrix((new THREE.Matrix4).makeRotationFromEuler(new THREE.Euler(Math.PI/2,Math.PI,0)));var camMaterial=new THREE.MeshNormalMaterial;var bHasCam=false;var obj,subObj,tarId;for(var _i=0,_l=scene.children.length;_i<_l;_i++){obj=scene.children[_i];if(obj==camera)bHasCam=true}if(!bHasCam)for(var _ii=0,_ll=scene.children.length;_ii<_ll;_ii++){obj=
scene.children[_ii];for(var _jj=0,lll=obj.children.length;_jj<lll;_jj++){subObj=obj.children[_jj];if(subObj==camera)tarId=_ii}}var spanM=_that._holoCamCenter.spanSphereMode;for(var i=0;i<npart;i++)for(var j=0;j<npart;j++){var mesh=new THREE.Mesh(camGeometry,camMaterial);var Gradient=new THREE.Vector3;var EachTarPos=new THREE.Vector3;var meshPosition=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanM);mesh.position.x=meshPosition.x;mesh.position.y=
meshPosition.y;mesh.position.z=meshPosition.z;mesh.lookAt(EachTarPos);this.camMeshs64.push(mesh);if(bHasCam||tarId==undefined)scene.add(mesh);else scene.children[tarId].add(mesh);var meshSX=mesh.clone();var meshPosSX=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanM,0.5,0);meshSX.position.x=meshPosSX.x;meshSX.position.y=meshPosSX.y;meshSX.position.z=meshPosSX.z;meshSX.lookAt(EachTarPos);this.camMeshs64.push(meshSX);var meshSY=mesh.clone();
var meshPosSY=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanM,0,-0.5);meshSY.position.x=meshPosSY.x;meshSY.position.y=meshPosSY.y;meshSY.position.z=meshPosSY.z;meshSY.lookAt(EachTarPos);this.camMeshs64.push(meshSY);var meshSXY=mesh.clone();var meshPosSXY=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanM,0.5,-0.5);meshSXY.position.x=meshPosSXY.x;meshSXY.position.y=meshPosSXY.y;meshSXY.position.z=
meshPosSXY.z;meshSXY.lookAt(EachTarPos);this.camMeshs64.push(meshSXY);if(bHasCam||tarId==undefined){scene.add(meshSX);scene.add(meshSY);scene.add(meshSXY)}else{scene.children[tarId].add(meshSX);scene.children[tarId].add(meshSY);scene.children[tarId].add(meshSXY)}if(_that.nShaderMode==0||(_that.nShaderMode==1||_that.nShaderMode==5)){meshSX.visible=false;meshSY.visible=false;meshSXY.visible=false}}this.ObjMesh2.push(_that._holoCamCenter.eyeCenter);this.ObjMesh2.push(_that._holoScreen.tarObj);this.camControls.attach(this.ObjMesh2[0],
false);this.tarControls.attach(this.ObjMesh2[1],false);this.nearPlaneControls.attach(_that._holoClipFrustum.nearPlane,false);this.farPlaneControls.attach(_that._holoClipFrustum.farPlane,false);this.ObjMesh2[0].visible=true;this.ObjMesh2[1].visible=true;if(bHasCam||tarId==undefined){scene.add(this.ObjMesh2[0]);scene.add(this.ObjMesh2[1]);scene.add(this.camControls);scene.add(this.tarControls);scene.add(this.nearPlaneControls);scene.add(this.farPlaneControls)}else{scene.children[tarId].add(this.ObjMesh2[0]);
scene.children[tarId].add(this.ObjMesh2[1]);scene.children[tarId].add(this.tarControls);scene.add(this.camControls);scene.add(this.nearPlaneControls);scene.add(this.farPlaneControls)}};this.init();this.update=function(){var spanMode=_that._holoCamCenter.spanSphereMode;var _left=Math.floor(_canvas.width*_that.GObserveView.left);var _bottom=Math.floor(_canvas.height*_that.GObserveView.bottom);var _width=Math.floor(_canvas.width*_that.GObserveView.width);var _height=Math.floor(_canvas.height*_that.GObserveView.height);
_that.setViewport(_left,_bottom,_width,_height);_that.setScissor(_left,_bottom,_width,_height);_that.enableScissorTest(true);_that.setClearColor((new THREE.Color).setRGB(0.11,0.12,0.18));this.Gcamera.aspect=_width/_height;this.Gcamera.updateProjectionMatrix();for(var i=0;i<npart;i++)for(var j=0;j<npart;j++){var Gradient=new THREE.Vector3;var EachTarPos=new THREE.Vector3;if(_that.nShaderMode==0||(_that.nShaderMode==1||_that.nShaderMode==5)){var meshPosition=_that.getCameraPosition(camera.position,
camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanMode);this.camMeshs64[(i*npart+j)*4+0].position.x=meshPosition.x;this.camMeshs64[(i*npart+j)*4+0].position.y=meshPosition.y;this.camMeshs64[(i*npart+j)*4+0].position.z=meshPosition.z;this.camMeshs64[(i*npart+j)*4+0].lookAt(EachTarPos);this.camMeshs64[(i*npart+j)*4+1].visible=false;this.camMeshs64[(i*npart+j)*4+2].visible=false;this.camMeshs64[(i*npart+j)*4+3].visible=false}else{this.camMeshs64[(i*npart+j)*4+1].visible=true;this.camMeshs64[(i*
npart+j)*4+2].visible=true;this.camMeshs64[(i*npart+j)*4+3].visible=true;var meshPosition=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanMode);this.camMeshs64[(i*npart+j)*4+0].position.x=meshPosition.x;this.camMeshs64[(i*npart+j)*4+0].position.y=meshPosition.y;this.camMeshs64[(i*npart+j)*4+0].position.z=meshPosition.z;this.camMeshs64[(i*npart+j)*4+0].lookAt(EachTarPos);var meshPosSX=_that.getCameraPosition(camera.position,camera.targetPosition,
camera.up,npart,i,j,Gradient,EachTarPos,spanMode,0.5,0);this.camMeshs64[(i*npart+j)*4+1].position.x=meshPosSX.x;this.camMeshs64[(i*npart+j)*4+1].position.y=meshPosSX.y;this.camMeshs64[(i*npart+j)*4+1].position.z=meshPosSX.z;this.camMeshs64[(i*npart+j)*4+1].lookAt(EachTarPos);var meshPosSY=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanMode,0,-0.5);this.camMeshs64[(i*npart+j)*4+2].position.x=meshPosSY.x;this.camMeshs64[(i*npart+j)*4+2].position.y=
meshPosSY.y;this.camMeshs64[(i*npart+j)*4+2].position.z=meshPosSY.z;this.camMeshs64[(i*npart+j)*4+2].lookAt(EachTarPos);var meshPosSXY=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanMode,0.5,-0.5);this.camMeshs64[(i*npart+j)*4+3].position.x=meshPosSXY.x;this.camMeshs64[(i*npart+j)*4+3].position.y=meshPosSXY.y;this.camMeshs64[(i*npart+j)*4+3].position.z=meshPosSXY.z;this.camMeshs64[(i*npart+j)*4+3].lookAt(EachTarPos)}}this.LocalControls.enabled=
...
-event.detail/3;if(_this.object.name=="eyeCenter")_this.view64fov+=delta*0.1;if(_this.object.name=="tarPlane"){_this.object.scale.x+=delta*0.003;_this.object.scale.y+=delta*0.003;_this.object.scale.z+=delta*0.003}}var getMouseOnScreen=function(){var vector=new THREE.Vector2;return function(layerX,layerY){vector.set((layerX-_this.screen.left)/_this.screen.width,(layerY-_this.screen.top)/_this.screen.height);return vector}}();function intersectObjs(pointer,objs){var _MousePos=new THREE.Vector2;_MousePos.copy(getMouseOnScreen(pointer.layerX,
