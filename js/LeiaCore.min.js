...
targetPosition.x;w1=mat11*u1+mat21*u2+targetPosition.y;w2=mat02*u0+mat12*u1+mat22*u2+targetPosition.z;var outPosition=new THREE.Vector3;outPosition.x=w0;outPosition.y=w1;outPosition.z=w2;var _eachTarPos=new THREE.Vector3(targetPosition.x,targetPosition.y,targetPosition.z);_eachTarPos.add(outPosition.clone().sub(position));EachTarPos.copy(_eachTarPos);return outPosition};this.getCameraIntrinsic=function(camera,_tarObj,_i,_j){var _local_lrbt=[];for(var i=0;i<4;i++){var point=new THREE.Vector3;_local_lrbt.push(point)}var camMat=
new THREE.Matrix4;camMat.getInverse(camera.matrix);camMat.multiply(_tarObj.matrix);for(var i=0;i<4;i++){var __point=new THREE.Vector3(_tarObj.geometry.vertices[i].x,_tarObj.geometry.vertices[i].y,_tarObj.geometry.vertices[i].z);_local_lrbt[i].copy(__point)}for(var i=0;i<4;i++)_local_lrbt[i].applyMatrix4(camMat);var n=camera.near;var f=camera.far;var d=0;if(_local_lrbt[0]!==undefined){var r=_local_lrbt[3].x;var l=_local_lrbt[2].x;var t=_local_lrbt[1].y;var b=_local_lrbt[3].y;d=-1*_local_lrbt[3].z;
var m11=2*d/(r-l);var m12=0;var m13=(r+l)/(r-l);var m14=0;var m21=0;var m22=2*d/(t-b);var m23=(t+b)/(t-b);var m24=0;var m31=0;var m32=0;var m33=(f+n)/(n-f);var m34=2*f*n/(n-f);var m41=0;var m42=0;var m43=-1;var m44=0;camera.projectionMatrix.set(m11,m12,m13,m14,m21,m22,m23,m24,m31,m32,m33,m34,m41,m42,m43,m44)}camera.near=d*0.6;camera.far=d*1.15;return d};this._holoScreen=undefined;this.bHoloScreenInit=false;var CHoloScreen=function(camera,_sizeX){this.position=new THREE.Vector3(0,0,0);this.position.copy(camera.targetPosition);
this.scale=1;var originSize=_sizeX;this.sizeX=_sizeX;var _length=this.sizeX/4;var geoTarRect=new THREE.PlaneGeometry(1*_length*4,1*_length*3,1,1);var matTarRect=new THREE.MeshBasicMaterial({color:26282,transparent:true,opacity:0.2});matTarRect.side=THREE.DoubleSide;this.tarObj=new THREE.Mesh(geoTarRect,matTarRect);this.tarObj.name="tarPlane";this.tarObj.visible=true;this.tarObj.rotation.setFromRotationMatrix(camera.matrix);this.tarObj.position.set(this.position.x,this.position.y,this.position.z);
this.tarObj.scale.x=this.scale;this.tarObj.scale.y=this.scale;this.tarObj.scale.z=this.scale;this.tarObj.updateMatrix();this.getData=function(){this.position.copy(this.tarObj.position);this.scale=this.tarObj.scale.x;this.sizeX=originSize*this.scale;this.tarObj.rotation.setFromRotationMatrix(camera.matrix)};this.setData=function(){this.tarObj.position.copy(this.position);this.tarObj.scale.x=this.scale;this.tarObj.scale.y=this.scale;this.tarObj.scale.z=this.scale;this.sizeX=originSize*this.scale;this.tarObj.rotation.setFromRotationMatrix(camera.matrix)}};
this._holoCamCenter=undefined;this.bHoloCamCenterInit=false;var CHoloCamCenter=function(camera,_fov){this.position=new THREE.Vector3;this.position.copy(camera.position);this.fov=_fov;this.spanSphereMode=false;var __point=new THREE.Vector3;__point.copy(camera.position.clone().sub(camera.targetPosition));var _length=__point.length()/2;var EyeCenterSize=_length/80;var geoEyeCenter=new THREE.SphereGeometry(EyeCenterSize,32,32);var matEyeCenter=new THREE.MeshBasicMaterial({color:16711935,transparent:false,
opacity:0.8});this.eyeCenter=new THREE.Mesh(geoEyeCenter,matEyeCenter);this.eyeCenter.position.set(this.position.x,this.position.y,this.position.z);this.eyeCenter.name="eyeCenter";this.eyeCenter.visible=true;this.eyeCenter.updateMatrix();this.getData=function(){this.position.copy(this.eyeCenter.position)};this.setData=function(){this.eyeCenter.position.copy(this.position)}};this.bGlobalViewInit=false;var _globalView;var CGlobalView=function(camera,scene,renderTarget,forceClear){var npart=8;this.camMeshs64=
[];this.ObjMesh2=[];this.Gcamera=new THREE.PerspectiveCamera(90,_canvas.width/_canvas.height,0.01,4E4);this.init=function(){var vecCend=new THREE.Vector3(camera.position.x,camera.position.y,camera.position.z);var vecGT=new THREE.Vector3((camera.position.x+camera.targetPosition.x)/2,(camera.position.y+camera.targetPosition.y)/2,(camera.position.z+camera.targetPosition.z)/2);vecCend.x-=vecGT.x;vecCend.y-=vecGT.y;vecCend.z-=vecGT.z;var vecUp=new THREE.Vector3(0,-vecCend.z,vecCend.y);var lengthVecUp=
Math.sqrt(vecUp.x*vecUp.x+vecUp.y*vecUp.y+vecUp.z*vecUp.z);vecUp.x/=lengthVecUp;vecUp.y/=lengthVecUp;vecUp.z/=lengthVecUp;var __length=Math.sqrt(vecCend.x*vecCend.x+vecCend.y*vecCend.y+vecCend.z*vecCend.z);vecUp.x=vecUp.x*2*__length+vecGT.x;vecUp.y=vecUp.y*2*__length+vecGT.y;vecUp.z=vecUp.z*2*__length+vecGT.z;this.Gcamera=new THREE.PerspectiveCamera(90,_canvas.width/_canvas.height,0.01,4E4);this.Gcamera.position.x=vecUp.x;this.Gcamera.position.y=vecUp.y;this.Gcamera.position.z=vecUp.z;this.Gcamera.up.x=
vecCend.x;this.Gcamera.up.y=vecCend.y;this.Gcamera.up.z=vecCend.z;this.Gcamera.lookAt(new THREE.Vector3(vecGT.x,vecGT.y,vecGT.z));this.LocalControls=new dragControls(this.Gcamera);this.LocalControls.screen.left=0;this.LocalControls.screen.top=_canvas.height*(1-_that.GObserveView.bottom-_that.GObserveView.height);this.LocalControls.screen.width=_canvas.width*_that.GObserveView.width;this.LocalControls.screen.height=_canvas.height*_that.GObserveView.height;this.camControls=new pickControls(this.Gcamera,
undefined,__length/5);this.camControls.view64fov=_that._holoCamCenter.fov;this.camControls.spanSphereMode=_that._holoCamCenter.spanSphereMode;this.camControls.screen.left=0;this.camControls.screen.top=_canvas.height*(1-_that.GObserveView.bottom-_that.GObserveView.height);this.camControls.screen.width=_canvas.width*_that.GObserveView.width;this.camControls.screen.height=_canvas.height*_that.GObserveView.height;this.tarControls=new pickControls(this.Gcamera,undefined,__length/2.5);this.tarControls.screen.left=
0;this.tarControls.screen.top=_canvas.height*(1-_that.GObserveView.bottom-_that.GObserveView.height);this.tarControls.screen.width=_canvas.width*_that.GObserveView.width;this.tarControls.screen.height=_canvas.height*_that.GObserveView.height;var camGeometry=new THREE.CylinderGeometry(0,__length/40,__length/20,20);camGeometry.applyMatrix((new THREE.Matrix4).makeRotationFromEuler(new THREE.Euler(Math.PI/2,Math.PI,0)));var camMaterial=new THREE.MeshNormalMaterial;var bHasCam=false;var obj,subObj,tarId;
for(var _i=0,_l=scene.children.length;_i<_l;_i++){obj=scene.children[_i];if(obj==camera)bHasCam=true}if(!bHasCam)for(var _ii=0,_ll=scene.children.length;_ii<_ll;_ii++){obj=scene.children[_ii];for(var _jj=0,lll=obj.children.length;_jj<lll;_jj++){subObj=obj.children[_jj];if(subObj==camera)tarId=_ii}}var spanM=_that._holoCamCenter.spanSphereMode;for(var i=0;i<npart;i++)for(var j=0;j<npart;j++){var mesh=new THREE.Mesh(camGeometry,camMaterial);var Gradient=new THREE.Vector3;var EachTarPos=new THREE.Vector3;
var meshPosition=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanM);mesh.position.x=meshPosition.x;mesh.position.y=meshPosition.y;mesh.position.z=meshPosition.z;mesh.lookAt(EachTarPos);this.camMeshs64.push(mesh);if(bHasCam||tarId==undefined)scene.add(mesh);else scene.children[tarId].add(mesh);var meshSX=mesh.clone();var meshPosSX=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanM,0.5,
0);meshSX.position.x=meshPosSX.x;meshSX.position.y=meshPosSX.y;meshSX.position.z=meshPosSX.z;meshSX.lookAt(EachTarPos);this.camMeshs64.push(meshSX);var meshSY=mesh.clone();var meshPosSY=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanM,0,-0.5);meshSY.position.x=meshPosSY.x;meshSY.position.y=meshPosSY.y;meshSY.position.z=meshPosSY.z;meshSY.lookAt(EachTarPos);this.camMeshs64.push(meshSY);var meshSXY=mesh.clone();var meshPosSXY=_that.getCameraPosition(camera.position,
camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanM,0.5,-0.5);meshSXY.position.x=meshPosSXY.x;meshSXY.position.y=meshPosSXY.y;meshSXY.position.z=meshPosSXY.z;meshSXY.lookAt(EachTarPos);this.camMeshs64.push(meshSXY);if(bHasCam||tarId==undefined){scene.add(meshSX);scene.add(meshSY);scene.add(meshSXY)}else{scene.children[tarId].add(meshSX);scene.children[tarId].add(meshSY);scene.children[tarId].add(meshSXY)}if(_that.nShaderMode==0||(_that.nShaderMode==1||_that.nShaderMode==5)){meshSX.visible=
false;meshSY.visible=false;meshSXY.visible=false}}this.ObjMesh2.push(_that._holoCamCenter.eyeCenter);this.ObjMesh2.push(_that._holoScreen.tarObj);this.camControls.attach(this.ObjMesh2[0],false);this.tarControls.attach(this.ObjMesh2[1],false);this.ObjMesh2[0].visible=false;this.ObjMesh2[1].visible=false;if(bHasCam||tarId==undefined){scene.add(this.ObjMesh2[0]);scene.add(this.ObjMesh2[1]);scene.add(this.camControls);scene.add(this.tarControls)}else{scene.children[tarId].add(this.ObjMesh2[0]);scene.children[tarId].add(this.ObjMesh2[1]);
scene.children[tarId].add(this.camControls);scene.children[tarId].add(this.tarControls)}};this.init();this.update=function(){var spanMode=_that._holoCamCenter.spanSphereMode;var _left=Math.floor(_canvas.width*_that.GObserveView.left);var _bottom=Math.floor(_canvas.height*_that.GObserveView.bottom);var _width=Math.floor(_canvas.width*_that.GObserveView.width);var _height=Math.floor(_canvas.height*_that.GObserveView.height);_that.setViewport(_left,_bottom,_width,_height);_that.setScissor(_left,_bottom,
_width,_height);_that.enableScissorTest(true);_that.setClearColor((new THREE.Color).setRGB(0.11,0.12,0.18));this.Gcamera.aspect=_width/_height;this.Gcamera.updateProjectionMatrix();for(var i=0;i<npart;i++)for(var j=0;j<npart;j++){var Gradient=new THREE.Vector3;var EachTarPos=new THREE.Vector3;if(_that.nShaderMode==0||(_that.nShaderMode==1||_that.nShaderMode==5)){var meshPosition=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanMode);this.camMeshs64[(i*
npart+j)*4+0].position.x=meshPosition.x;this.camMeshs64[(i*npart+j)*4+0].position.y=meshPosition.y;this.camMeshs64[(i*npart+j)*4+0].position.z=meshPosition.z;this.camMeshs64[(i*npart+j)*4+0].lookAt(EachTarPos);this.camMeshs64[(i*npart+j)*4+1].visible=false;this.camMeshs64[(i*npart+j)*4+2].visible=false;this.camMeshs64[(i*npart+j)*4+3].visible=false}else{this.camMeshs64[(i*npart+j)*4+1].visible=true;this.camMeshs64[(i*npart+j)*4+2].visible=true;this.camMeshs64[(i*npart+j)*4+3].visible=true;var meshPosition=
_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanMode);this.camMeshs64[(i*npart+j)*4+0].position.x=meshPosition.x;this.camMeshs64[(i*npart+j)*4+0].position.y=meshPosition.y;this.camMeshs64[(i*npart+j)*4+0].position.z=meshPosition.z;this.camMeshs64[(i*npart+j)*4+0].lookAt(EachTarPos);var meshPosSX=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanMode,0.5,0);this.camMeshs64[(i*npart+j)*
4+1].position.x=meshPosSX.x;this.camMeshs64[(i*npart+j)*4+1].position.y=meshPosSX.y;this.camMeshs64[(i*npart+j)*4+1].position.z=meshPosSX.z;this.camMeshs64[(i*npart+j)*4+1].lookAt(EachTarPos);var meshPosSY=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanMode,0,-0.5);this.camMeshs64[(i*npart+j)*4+2].position.x=meshPosSY.x;this.camMeshs64[(i*npart+j)*4+2].position.y=meshPosSY.y;this.camMeshs64[(i*npart+j)*4+2].position.z=meshPosSY.z;this.camMeshs64[(i*
npart+j)*4+2].lookAt(EachTarPos);var meshPosSXY=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanMode,0.5,-0.5);this.camMeshs64[(i*npart+j)*4+3].position.x=meshPosSXY.x;this.camMeshs64[(i*npart+j)*4+3].position.y=meshPosSXY.y;this.camMeshs64[(i*npart+j)*4+3].position.z=meshPosSXY.z;this.camMeshs64[(i*npart+j)*4+3].lookAt(EachTarPos)}}this.LocalControls.enabled=true;if(this.tarControls.axis!=null||this.camControls.axis!=null)this.LocalControls.enabled=
...
_that.GGyroSimView.height);_that.setViewport(_left,_bottom,_width,_height);_that.setScissor(_left,_bottom,_width,_height);_that.enableScissorTest(true);_that.setClearColor((new THREE.Color).setRGB(0.11,0.12,0.18));this.GyroSimCam.aspect=_width/_height;this.GyroSimCam.updateProjectionMatrix();this.localSim.update();if(_that.bGyroSimView)_that.render(this.GyroSimScene,this.GyroSimCam,renderTarget,forceClear)}};var Leia_compute_renderViews=function(scene,camera,renderTarget,forceClear,shiftX,shiftY,
_npart){var spanMode=_that._holoCamCenter.spanSphereMode;_that._holoCamCenter.eyeCenter.updateMatrix();var camPositionCenter=new THREE.Vector3(_that._holoCamCenter.eyeCenter.position.x,_that._holoCamCenter.eyeCenter.position.y,_that._holoCamCenter.eyeCenter.position.z);var tmpM=new THREE.Matrix4;var tmpV=new THREE.Vector3(camPositionCenter.x-camera.targetPosition.x,camPositionCenter.y-camera.targetPosition.y,camPositionCenter.z-camera.targetPosition.z);var npart=8;if(_npart!==undefined)npart=_npart;
var _d=0;if(shiftX==undefined)shiftX=0;if(shiftY==undefined)shiftY=0;camera.updateMatrix();_that._holoScreen.tarObj.rotation.setFromRotationMatrix(camera.matrix);_that._holoScreen.tarObj.updateMatrix();for(var ii=0;ii<npart;ii++)for(var jj=0;jj<npart;jj++){if(renderTarget!==undefined){_that.setViewport(renderTarget.width/npart*ii,renderTarget.height/npart*jj,renderTarget.width/npart,renderTarget.height/npart);_that.setScissor(renderTarget.width/npart*ii,renderTarget.height/npart*jj,renderTarget.width/
npart,renderTarget.height/npart)}else{_that.setViewport(_canvas.width/npart*ii,_canvas.height/npart*jj,_canvas.width/npart,_canvas.height/npart);_that.setScissor(_viewportWidth/npart*ii,_viewportHeight/npart*jj,_viewportWidth/npart,_viewportHeight/npart)}_that.enableScissorTest(true);var Gradient=new THREE.Vector3;var EachTarPos=new THREE.Vector3;var camPosition=_that.getCameraPosition(camPositionCenter,camera.targetPosition,camera.up,npart,ii,jj,Gradient,EachTarPos,spanMode,shiftX,shiftY);camera.position.x=
camPosition.x;camera.position.y=camPosition.y;camera.position.z=camPosition.z;tmpM.lookAt(camera.position,EachTarPos,camera.up);camera.quaternion.setFromRotationMatrix(tmpM);camera.updateMatrix();if(_that._holoScreen.tarObj.geometry.vertices[0]!==undefined)_d=_that.getCameraIntrinsic(camera,_that._holoScreen.tarObj,ii,jj);if(renderTarget!==undefined){renderTarget.sx=renderTarget.width/npart*ii;renderTarget.sy=renderTarget.height/npart*jj;renderTarget.w=renderTarget.width/npart;renderTarget.h=renderTarget.height/
npart}_that.render(scene,camera,renderTarget,forceClear)}camera.position.x=camPositionCenter.x;camera.position.y=camPositionCenter.y;camera.position.z=camPositionCenter.z;camera.up.set(0,1,0);if(tmpV.y>0&&(tmpV.y>=Math.abs(tmpV.x)*2&&tmpV.y>=Math.abs(tmpV.z)*2))camera.up.set(0,0,-1);camera.lookAt(camera.targetPosition)};this.stateData={};this.messageFlag=0;this.SetUpRenderStates=function(scene,camera,renderTarget,forceClear,holoScreenSize,holoCamFov,messageFlag){var _holoCamFov=50;var _holoScreenScale=
1;if(holoCamFov!==undefined)_holoCamFov=holoCamFov;var _holoScreenSize=100;if(holoScreenSize!==undefined)_holoScreenSize=holoScreenSize;if(camera.position.x==0&&(camera.position.y!=0&&camera.position.z==0))camera.position.z=camera.position.y/100;if(!this.bHoloCamCenterInit){this._holoCamCenter=new CHoloCamCenter(camera,_holoCamFov);this.bHoloCamCenterInit=true;this.stateData._camFov=this._holoCamCenter.fov;this.stateData._camPosition=new THREE.Vector3(0,0,0);this.stateData._camPosition.copy(this._holoCamCenter.position)}if(!this.bHoloScreenInit&&
camera.position.length()>=0){this._holoScreen=new CHoloScreen(camera,_holoScreenSize);this.bHoloScreenInit=true;this.stateData._holoScreenScale=this._holoScreen.scale;this.stateData._holoScreenSize=this._holoScreen.sizeX;this.stateData._tarPosition=new THREE.Vector3(0,0,0);this.stateData._tarPosition.copy(this._holoScreen.position);scene.add(this._holoScreen.tarObj);this._holoScreen.tarObj.visible=false}if(!this.bShaderManInit){this._shaderManager=new CShaderManager(_that,_viewportWidth,_viewportHeight);
this.bShaderManInit=true}if(messageFlag==undefined)console.log("messageFlag undefined");else if(messageFlag==0){this._holoScreen.getData();this._holoCamCenter.getData();var bStateChange=false;if(this.stateData._camFov!=this._holoCamCenter.fov||this.stateData._holoScreenScale!=this._holoScreen.scale)bStateChange=true;if(this.stateData._camPosition.x!=this._holoCamCenter.position.x||(this.stateData._camPosition.y!=this._holoCamCenter.position.y||this.stateData._camPosition.z!=this._holoCamCenter.position.z))bStateChange=
true;if(this.stateData._tarPosition.x!=this._holoScreen.position.x||(this.stateData._tarPosition.y!=this._holoScreen.position.y||this.stateData._tarPosition.z!=this._holoScreen.position.z))bStateChange=true;if(bStateChange==true){var message=JSON.stringify({type:"tuning",data:{_camFov:this._holoCamCenter.fov,_camPosition:{x:this._holoCamCenter.position.x,y:this._holoCamCenter.position.y,z:this._holoCamCenter.position.z},_holoScreenScale:this._holoScreen.scale,_holoScreenSize:this._holoScreen.sizeX,
_tarPosition:{x:this._holoScreen.position.x,y:this._holoScreen.position.y,z:this._holoScreen.position.z}}});window.top.postMessage(message,"*");this.stateData._camFov=this._holoCamCenter.fov;this.stateData._camPosition.copy(this._holoCamCenter.position);this.stateData._holoScreenScale=this._holoScreen.scale;this.stateData._holoScreenSize=this._holoScreen.sizeX;this.stateData._tarPosition.copy(this._holoScreen.position)}var self=this;if(bStateChange==true){console.log("post data to emulator");console.log("self._holoScreen.sizeX:"+
self._holoScreen.sizeX);(function(){var dataObject={action:"UpdateDisplayParams"};dataObject.params=JSON.stringify({type:"tuning",data:{_camFov:self._holoCamCenter.fov,_camPosition:{x:self._holoCamCenter.position.x,y:self._holoCamCenter.position.y,z:self._holoCamCenter.position.z},_holoScreenScale:self._holoScreen.scale,_holoScreenSize:self._holoScreen.sizeX,_tarPosition:{x:self._holoScreen.position.x,y:self._holoScreen.position.y,z:self._holoScreen.position.z}}});var xmlhttp=new XMLHttpRequest;xmlhttp.onreadystatechange=
function(){if(this.readyState==this.DONE)if(this.status==200&&this.response!=null){var params=JSON.parse(this.responseText);console.log("Update Display Params:"+this.responseText);return}};xmlhttp.open("POST","http://127.0.0.1:8887/updateDisplayParams",true);xmlhttp.setRequestHeader("Content-Type","text/plain");xmlhttp.send(JSON.stringify(dataObject))})()}}else if(messageFlag==1){this._holoScreen.setData();this._holoCamCenter.setData();this.messageFlag++;if(this.messageFlag>5){this.messageFlag=0;
var self=this;(function(){var xmlhttp=new XMLHttpRequest;xmlhttp.onreadystatechange=function(){if(this.readyState==this.DONE)if(this.status==200&&this.response!=null){var params=JSON.parse(this.responseText);if(params.data!=undefined&&params.type=="tuning"){self._holoCamCenter.fov=params.data._camFov.toFixed(2);self._holoCamCenter.position.x=params.data._camPosition.x.toFixed(2);self._holoCamCenter.position.y=params.data._camPosition.y.toFixed(2);self._holoCamCenter.position.z=params.data._camPosition.z.toFixed(2);
self._holoCamCenter.setData();self._holoScreen.scale=params.data._holoScreenScale.toFixed(2);self._holoScreen.sizeX=params.data._holoScreenSize.toFixed(2);self._holoScreen.position.x=params.data._tarPosition.x;self._holoScreen.position.y=params.data._tarPosition.y;self._holoScreen.position.z=params.data._tarPosition.z;self._holoScreen.setData()}return}else console.log("something wrong")};xmlhttp.open("GET","http://127.0.0.1:8887/queryDisplayParams",true);xmlhttp.send()})()}}else console.log("messageFlag Error!")};
this.bRendering=true;this.material_depth=new THREE.MeshDepthMaterial;this.Leia_render=function(scene,camera,renderTarget,forceClear,holoScreenSize,holoCamFov,messageFlag){scene.overrideMaterial=null;this.SetUpRenderStates(scene,camera,renderTarget,forceClear,holoScreenSize,holoCamFov,messageFlag);if(this.bRendering){if(this.messageFlag!==0||this.messageFlag==0&&(this.bGlobalView==false&&this.bGyroSimView==false))if(0==this._renderMode){var camPositionCenter=new THREE.Vector3(camera.position.x,camera.position.y,
camera.position.z);var tmpM=new THREE.Matrix4;var tmpV=new THREE.Vector3(camPositionCenter.x-camera.targetPosition.x,camPositionCenter.y-camera.targetPosition.y,camPositionCenter.z-camera.targetPosition.z);var _d=0;this.setViewport(0,0,_canvas.width,_canvas.height);this.setScissor(0,0,_viewportWidth,_viewportHeight);this.enableScissorTest(true);camera.up.set(0,1,0);if(tmpV.y>0&&(tmpV.y>=Math.abs(tmpV.x)*2&&tmpV.y>=Math.abs(tmpV.z)*2))camera.up.set(0,0,-1);if(_that._holoScreen.tarObj.geometry.vertices[0]!==
undefined)_d=this.getCameraIntrinsic(camera,_that._holoScreen.tarObj);scene.overrideMaterial=_that.material_depth;this.render(scene,camera,renderTarget,forceClear)}else if(1==this._renderMode){console.log("render64");Leia_compute_renderViews(scene,camera,renderTarget,forceClear);if(this.nShaderMode==2||(this.nShaderMode==3||this.nShaderMode==4)){Leia_compute_renderViews(scene,camera,renderTarget,forceClear,0.5,0);Leia_compute_renderViews(scene,camera,renderTarget,forceClear,0,-0.5);Leia_compute_renderViews(scene,
camera,renderTarget,forceClear,0.5,-0.5)}}else if(2==this._renderMode){if(this.nShaderMode==0||(this.nShaderMode==1||(this.nShaderMode==2||(this.nShaderMode==3||this.nShaderMode==5)))){Leia_compute_renderViews(scene,camera,this._shaderManager._swizzleRenderTarget,forceClear);if(this.nShaderMode==2||this.nShaderMode==3){Leia_compute_renderViews(scene,camera,this._shaderManager._swizzleRenderTargetSftX,forceClear,0.5,0);Leia_compute_renderViews(scene,camera,this._shaderManager._swizzleRenderTargetSftY,
forceClear,0,-0.5);Leia_compute_renderViews(scene,camera,this._shaderManager._swizzleRenderTargetSftXY,forceClear,0.5,-0.5)}}if(this.nShaderMode==4)Leia_compute_renderViews(scene,camera,this._shaderManager._swizzleRenderTargetSSS,forceClear,0,0,16);if(this.nShaderMode==5){scene.overrideMaterial=_that.material_depth;Leia_compute_renderViews(scene,camera,this._shaderManager._DepthRenderTarget,true)}this.setViewport(0,0,_canvas.width,_canvas.height);this.setScissor(0,0,_viewportWidth,_viewportHeight);
this.enableScissorTest(true);_that.render(this._shaderManager.LEIA_output,this._shaderManager.cameraSWIZZLE)}else console.log("renderMode error!");if(this.bGlobalView)if(!this.bGlobalViewInit){_globalView=new CGlobalView(camera,scene,renderTarget,forceClear);this.bGlobalViewInit=true}else _globalView.update();if(this.bGyroSimView)if(!this.bGyroSimViewInit){_gyroView=new CGyroView(renderTarget,forceClear);this.bGyroSimViewInit=true}else _gyroView.update()}}};LeiaWebGLRenderer.prototype=Object.create(THREE.WebGLRenderer.prototype);var CShaderManager=function(leia_renderer,_viewportWidth,_viewportHeight){this.render=leia_renderer;this._swizzleRenderTarget=undefined;this.cameraSWIZZLE=undefined;this.LEIA_output;this.swizzleMesh;this.materialSwizzle;this.matBasic;this.matSharpen;this.matSuperSample;this.matSSS;this.matSSSTest;this.matDepth;this._swizzleRenderTargetSftX;this._swizzleRenderTargetSftY;this._swizzleRenderTargetSftXY;this._swizzleRenderTargetSSS;this._DepthRenderTarget;this.width=_viewportWidth;this.height=_viewportHeight;
this.cameraSWIZZLE=new THREE.OrthographicCamera(this.width/-2,this.width/2,this.height/2,this.height/-2,-1,1);this.cameraSWIZZLE.position.z=0;this.LEIA_output=new THREE.Scene;if(this.LEIA_output.children.length>0)this.LEIA_output.remove(this.swizzleMesh);var swizzleBackgroundGeometry=new THREE.PlaneGeometry(this.width,this.height);var _SwizzleVertexShaderSrc="varying vec2 vUv;"+"void main() {"+"    vUv = uv;"+"    gl_Position = projectionMatrix * modelViewMatrix * vec4( position, 1.0 );"+"}";var _SwizzleFragmentShaderSrc=
...
"return pb;\n"+"}\n"+"void main(void) {\t\t\t\t\t\t\n"+"vec2 pixelCoord = vec2( floor((vUv.s)*renderSize.x), floor(vUv.t*renderSize.y) ); "+"pixelCoord      = vec2(max(pixelCoord.s - 0.0, 0.0), max(pixelCoord.t - 0.0, 0.0));"+"vec2 viewId     = vec2(   mod(pixelCoord.s,8.0)  ,   mod(pixelCoord.t,8.0)  ); "+"vec2 sPixId     = vec2( floor(pixelCoord.s/8.0)  , floor(pixelCoord.t/8.0)  ); "+"float fc        = 0.0;"+"fc = getPixel( 1.0, tNormal, viewId, sPixId);"+"gl_FragColor = vec4(fc, fc, fc, 1.0);"+
"}";var invA=[1.1146,-0.1909,0.0343,0,0,0];function LEIA_internal_fragmentShaderFunction_getSharpPixel5(){var snipplet;var B1X=8-1;var B1Y=8-1;var B2X=8-2;var B2Y=8-2;snipplet="uniform float invA [6]; \n";snipplet+=false?"vec4":"float";snipplet+=" getSharpPixel( in float amplitudes [6], in sampler2D texture, in vec2 viewId, in vec2 sPixId) { \n";snipplet+="    ";snipplet+="    float s1m = viewId.s - 1.0;\n";snipplet+="    float s1p = viewId.s + 1.0;\n";snipplet+="    float t1m = viewId.t - 1.0;\n";
snipplet+="    float t1p = viewId.t + 1.0;\n";snipplet+="    float s2m = viewId.s - 2.0;\n";snipplet+="    float s2p = viewId.s + 2.0;\n";snipplet+="    float t2m = viewId.t - 2.0;\n";snipplet+="    float t2p = viewId.t + 2.0;\n";snipplet+="    ";snipplet+=false?"vec4":"float";snipplet+=" p = getPixel( amplitudes[0], texture, viewId, sPixId);\n";snipplet+="    float q = amplitudes[0];\n";snipplet+="    if (viewId.s > 0.0) { \n";snipplet+="        p += getPixel( amplitudes[1], texture, vec2( s1m, viewId.t ), sPixId );\n";
...
"vec4 p   = texture2D( texture, id );\n"+"float pb = amplitude * ( p.r + p.g + p.b ) / 3.0;\n"+"return pb;\n"+"}\n"+LEIA_internal_fragmentShaderFunction_getSharpPixel5()+"void main(void) {\t\t\t\t\t\t\n"+"vec2 pixelCoord = vec2( floor((vUv.s)*renderSize.x), floor(vUv.t*renderSize.y) ); "+"pixelCoord      = vec2(max(pixelCoord.s - 0.0, 0.0), max(pixelCoord.t - 0.0, 0.0));"+"vec2 viewId     = vec2(   mod(pixelCoord.s,8.0)  ,   mod(pixelCoord.t,8.0)  ); "+"vec2 sPixId     = vec2( floor(pixelCoord.s/8.0)  , floor(pixelCoord.t/8.0)  ); "+
"float fc        = 0.0;"+"fc = getSharpPixel( invA, tNormal, viewId, sPixId);\n"+"gl_FragColor = vec4(fc, fc, fc, 1.0);"+"}";var _SuperSampleSwizzleFragmentShaderSrc="precision highp float;"+"varying  vec2 vUv; \t\t\t\n"+"uniform sampler2D tNormal; \t\t\t\n"+"uniform sampler2D tSuperX; \t\t\t\n"+"uniform sampler2D tSuperY; \t\t\t\n"+"uniform sampler2D tSuperD; \t\t\t\n"+"uniform vec2 renderSize;              \n "+"float getPixel( in float amplitude, in sampler2D texture, in vec2 viewId, in vec2 sPixId) {  \n"+
"vec2 id  = vec2( ( sPixId.s + viewId.s*renderSize.x/8.0 )/renderSize.x + 1.0/(2.0*renderSize.x), ( sPixId.t + viewId.t*renderSize.y/8.0 )/renderSize.y+ 1.0/(2.0*renderSize.y) ); \n"+"vec4 p   = texture2D( texture, id );\n"+"float pb = amplitude * ( p.r + p.g + p.b ) / 3.0;\n"+"return pb;\n"+"}\n"+"void main(void) {\t\t\t\t\t\t\n"+"vec2 pixelCoord = vec2( floor((vUv.s)*renderSize.x), floor(vUv.t*renderSize.y) ); "+"pixelCoord      = vec2(max(pixelCoord.s - 0.0, 0.0), max(pixelCoord.t - 0.0, 0.0));"+
"vec2 viewId     = vec2(   mod(pixelCoord.s,8.0)  ,   mod(pixelCoord.t,8.0)  ); "+"vec2 sPixId     = vec2( floor(pixelCoord.s/8.0)  , floor(pixelCoord.t/8.0)  ); "+"float fc        = 0.0;"+"fc = getPixel( 1.0, tNormal, viewId, sPixId);"+"float imgCoeff = 1.0;"+"float nnCoeff = 0.2;"+"float nxnCoeff = 0.1;"+"float coeff = imgCoeff+2.0*nnCoeff+nxnCoeff;"+"fc = getPixel(imgCoeff, tNormal, viewId, sPixId);"+"fc = fc+getPixel( nnCoeff, tSuperX, viewId, sPixId );"+"fc = fc+getPixel( nnCoeff, tSuperY, viewId, sPixId );"+
"fc = fc+getPixel( nxnCoeff, tSuperD, viewId, sPixId );"+"if (viewId.s > 0.0) { \n"+"   coeff = coeff + nnCoeff + nxnCoeff;"+"   fc = fc+getPixel( nnCoeff, tSuperX, viewId-vec2(1.0, 0.0), sPixId );"+"   fc = fc+getPixel( nxnCoeff, tSuperD, viewId-vec2(1.0, 0.0), sPixId );"+"}\n"+"if (viewId.t > 0.0) { \n"+"   coeff = coeff + nnCoeff + nxnCoeff;"+"   fc = fc+getPixel( nnCoeff, tSuperY, viewId-vec2(0.0, 1.0), sPixId );"+"   fc = fc+getPixel( nxnCoeff, tSuperD, viewId-vec2(0.0, 1.0), sPixId );"+"   if (viewId.s > 0.0) { \n"+
"       coeff = coeff + nxnCoeff;"+"       fc = fc+getPixel( nxnCoeff, tSuperD, viewId-vec2(1.0, 1.0), sPixId );"+"   }\n"+"}\n"+"fc = fc/coeff;"+"gl_FragColor = vec4(fc, fc, fc, 1.0);"+"}";var invA_66=[-0.0197,-0.0271,0.0864,0.146,0.0306,-0.1038,-0.0271,-0.0373,0.1189,0.2011,0.0422,-0.143,0.0393,0.0542,-0.0946,-0.1845,-0.4679,-0.4598,0.0813,0.1119,-0.2493,-0.4552,-0.6865,-0.4902,0.0545,0.075,-0.3056,-0.4959,1.2043,2.1543,-0.0063,-0.0086,-0.1716,-0.2276,2.3449,3.4569];var _SSSSwizzleFragmentShaderSrc=
"precision highp float;"+"varying  vec2 vUv; \t\t\t\n"+"uniform sampler2D tNormal; \t\t\t\n"+"uniform sampler2D tSuperX; \t\t\t\n"+"uniform sampler2D tSuperY; \t\t\t\n"+"uniform sampler2D tSuperD; \t\t\t\n"+"uniform vec2 renderSize;              \n "+"uniform float invASSS[36];                \n"+"float getPixel( in float amplitude, in sampler2D texture, in vec2 viewId, in vec2 sPixId) {  \n"+"vec2 id  = vec2( ( sPixId.s + viewId.s*renderSize.x/8.0 )/renderSize.x + 1.0/(2.0*renderSize.x), ( sPixId.t + viewId.t*renderSize.y/8.0 )/renderSize.y+ 1.0/(2.0*renderSize.y) ); \n"+
"vec4 p   = texture2D( texture, id );\n"+"float pb = amplitude * ( p.r + p.g + p.b ) / 3.0;\n"+"return pb;\n"+"}\n"+"void main(void) {\t\t\t\t\t\t\n"+"vec2 pixelCoord = vec2( floor((vUv.s)*renderSize.x), floor(vUv.t*renderSize.y) ); "+"pixelCoord      = vec2(max(pixelCoord.s - 0.0, 0.0), max(pixelCoord.t - 0.0, 0.0));"+"vec2 viewId     = vec2(   mod(pixelCoord.s,8.0)  ,   mod(pixelCoord.t,8.0)  ); "+"vec2 sPixId     = vec2( floor(pixelCoord.s/8.0)  , floor(pixelCoord.t/8.0)  ); "+"float fc        = 0.0;"+
"float coeff = 0.0;"+"if (viewId.s>=3.00){if (viewId.t>=3.00){ coeff = coeff+invASSS[0*6 + 0];fc = fc+getPixel(invASSS[0*6 + 0],tSuperD, vec2(viewId.s-3.00, viewId.t-3.00), sPixId);}};\n"+"if (viewId.s>=2.00){if (viewId.t>=3.00){ coeff = coeff+invASSS[0*6 + 1];fc = fc+getPixel(invASSS[0*6 + 1],tSuperY, vec2(viewId.s-2.00, viewId.t-3.00), sPixId);}};\n"+"if (viewId.s>=2.00){if (viewId.t>=3.00){ coeff = coeff+invASSS[0*6 + 2];fc = fc+getPixel(invASSS[0*6 + 2],tSuperD, vec2(viewId.s-2.00, viewId.t-3.00), sPixId);}};\n"+
