...
this.position.y,this.position.z);this.eyeCenter.name="eyeCenter";this.eyeCenter.visible=true;this.eyeCenter.updateMatrix();this.getData=function(){this.position.copy(this.eyeCenter.position)};this.setData=function(){this.eyeCenter.position.copy(this.position)}};this.bGlobalViewInit=false;var _globalView;var CGlobalView=function(camera,scene,renderTarget,forceClear){var npart=8;this.camMeshs64=[];this.ObjMesh2=[];this.Gcamera=new THREE.PerspectiveCamera(90,_canvas.width/_canvas.height,0.01,4E4);this.init=
...
var __length=Math.sqrt(vecCend.x*vecCend.x+vecCend.y*vecCend.y+vecCend.z*vecCend.z);vecUp.x=vecUp.x*2*__length+vecGT.x;vecUp.y=vecUp.y*2*__length+vecGT.y;vecUp.z=vecUp.z*2*__length+vecGT.z;this.Gcamera=new THREE.PerspectiveCamera(90,_canvas.width/_canvas.height,0.01,4E4);this.Gcamera.position.x=vecUp.x;this.Gcamera.position.y=vecUp.y;this.Gcamera.position.z=vecUp.z;this.Gcamera.up.x=vecCend.x;this.Gcamera.up.y=vecCend.y;this.Gcamera.up.z=vecCend.z;this.Gcamera.lookAt(new THREE.Vector3(vecGT.x,vecGT.y,
...
_canvas.width*_that.GObserveView.width;this.tarControls.screen.height=_canvas.height*_that.GObserveView.height;var camGeometry=new THREE.CylinderGeometry(0,__length/40,__length/20,20);camGeometry.applyMatrix((new THREE.Matrix4).makeRotationFromEuler(new THREE.Euler(Math.PI/2,Math.PI,0)));var camMaterial=new THREE.MeshNormalMaterial;var bHasCam=false;var obj,subObj,tarId;for(var _i=0,_l=scene.children.length;_i<_l;_i++){obj=scene.children[_i];if(obj==camera)bHasCam=true}if(!bHasCam)for(var _ii=0,_ll=
scene.children.length;_ii<_ll;_ii++){obj=scene.children[_ii];for(var _jj=0,lll=obj.children.length;_jj<lll;_jj++){subObj=obj.children[_jj];if(subObj==camera)tarId=_ii}}var spanM=_that._holoCamCenter.spanSphereMode;for(var i=0;i<npart;i++)for(var j=0;j<npart;j++){var mesh=new THREE.Mesh(camGeometry,camMaterial);var Gradient=new THREE.Vector3;var EachTarPos=new THREE.Vector3;var meshPosition=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanM);
mesh.position.x=meshPosition.x;mesh.position.y=meshPosition.y;mesh.position.z=meshPosition.z;mesh.lookAt(EachTarPos);this.camMeshs64.push(mesh);if(bHasCam||tarId==undefined)scene.add(mesh);else scene.children[tarId].add(mesh);var meshSX=mesh.clone();var meshPosSX=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanM,0.5,0);meshSX.position.x=meshPosSX.x;meshSX.position.y=meshPosSX.y;meshSX.position.z=meshPosSX.z;meshSX.lookAt(EachTarPos);this.camMeshs64.push(meshSX);
var meshSY=mesh.clone();var meshPosSY=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanM,0,-0.5);meshSY.position.x=meshPosSY.x;meshSY.position.y=meshPosSY.y;meshSY.position.z=meshPosSY.z;meshSY.lookAt(EachTarPos);this.camMeshs64.push(meshSY);var meshSXY=mesh.clone();var meshPosSXY=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanM,0.5,-0.5);meshSXY.position.x=meshPosSXY.x;meshSXY.position.y=
meshPosSXY.y;meshSXY.position.z=meshPosSXY.z;meshSXY.lookAt(EachTarPos);this.camMeshs64.push(meshSXY);if(bHasCam||tarId==undefined){scene.add(meshSX);scene.add(meshSY);scene.add(meshSXY)}else{scene.children[tarId].add(meshSX);scene.children[tarId].add(meshSY);scene.children[tarId].add(meshSXY)}if(_that.nShaderMode==0||(_that.nShaderMode==1||_that.nShaderMode==5)){meshSX.visible=false;meshSY.visible=false;meshSXY.visible=false}}this.ObjMesh2.push(_that._holoCamCenter.eyeCenter);this.ObjMesh2.push(_that._holoScreen.tarObj);
this.camControls.attach(this.ObjMesh2[0],false);this.tarControls.attach(this.ObjMesh2[1],false);this.ObjMesh2[0].visible=true;this.ObjMesh2[1].visible=true;if(bHasCam||tarId==undefined){scene.add(this.ObjMesh2[0]);scene.add(this.ObjMesh2[1]);scene.add(this.camControls);scene.add(this.tarControls)}else{scene.children[tarId].add(this.ObjMesh2[0]);scene.children[tarId].add(this.ObjMesh2[1]);scene.children[tarId].add(this.camControls);scene.children[tarId].add(this.tarControls)}};this.init();this.update=
function(){var spanMode=_that._holoCamCenter.spanSphereMode;var _left=Math.floor(_canvas.width*_that.GObserveView.left);var _bottom=Math.floor(_canvas.height*_that.GObserveView.bottom);var _width=Math.floor(_canvas.width*_that.GObserveView.width);var _height=Math.floor(_canvas.height*_that.GObserveView.height);_that.setViewport(_left,_bottom,_width,_height);_that.setScissor(_left,_bottom,_width,_height);_that.enableScissorTest(true);_that.setClearColor((new THREE.Color).setRGB(0.11,0.12,0.18));this.Gcamera.aspect=
_width/_height;this.Gcamera.updateProjectionMatrix();for(var i=0;i<npart;i++)for(var j=0;j<npart;j++){var Gradient=new THREE.Vector3;var EachTarPos=new THREE.Vector3;if(_that.nShaderMode==0||(_that.nShaderMode==1||_that.nShaderMode==5)){var meshPosition=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanMode);this.camMeshs64[(i*npart+j)*4+0].position.x=meshPosition.x;this.camMeshs64[(i*npart+j)*4+0].position.y=meshPosition.y;this.camMeshs64[(i*
npart+j)*4+0].position.z=meshPosition.z;this.camMeshs64[(i*npart+j)*4+0].lookAt(EachTarPos);this.camMeshs64[(i*npart+j)*4+1].visible=false;this.camMeshs64[(i*npart+j)*4+2].visible=false;this.camMeshs64[(i*npart+j)*4+3].visible=false}else{this.camMeshs64[(i*npart+j)*4+1].visible=true;this.camMeshs64[(i*npart+j)*4+2].visible=true;this.camMeshs64[(i*npart+j)*4+3].visible=true;var meshPosition=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanMode);
this.camMeshs64[(i*npart+j)*4+0].position.x=meshPosition.x;this.camMeshs64[(i*npart+j)*4+0].position.y=meshPosition.y;this.camMeshs64[(i*npart+j)*4+0].position.z=meshPosition.z;this.camMeshs64[(i*npart+j)*4+0].lookAt(EachTarPos);var meshPosSX=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanMode,0.5,0);this.camMeshs64[(i*npart+j)*4+1].position.x=meshPosSX.x;this.camMeshs64[(i*npart+j)*4+1].position.y=meshPosSX.y;this.camMeshs64[(i*npart+j)*
4+1].position.z=meshPosSX.z;this.camMeshs64[(i*npart+j)*4+1].lookAt(EachTarPos);var meshPosSY=_that.getCameraPosition(camera.position,camera.targetPosition,camera.up,npart,i,j,Gradient,EachTarPos,spanMode,0,-0.5);this.camMeshs64[(i*npart+j)*4+2].position.x=meshPosSY.x;this.camMeshs64[(i*npart+j)*4+2].position.y=meshPosSY.y;this.camMeshs64[(i*npart+j)*4+2].position.z=meshPosSY.z;this.camMeshs64[(i*npart+j)*4+2].lookAt(EachTarPos);var meshPosSXY=_that.getCameraPosition(camera.position,camera.targetPosition,
camera.up,npart,i,j,Gradient,EachTarPos,spanMode,0.5,-0.5);this.camMeshs64[(i*npart+j)*4+3].position.x=meshPosSXY.x;this.camMeshs64[(i*npart+j)*4+3].position.y=meshPosSXY.y;this.camMeshs64[(i*npart+j)*4+3].position.z=meshPosSXY.z;this.camMeshs64[(i*npart+j)*4+3].lookAt(EachTarPos)}}this.LocalControls.enabled=true;if(this.tarControls.axis!=null||this.camControls.axis!=null)this.LocalControls.enabled=false;this.LocalControls.screen.left=0;this.LocalControls.screen.top=_canvas.height*(1-_that.GObserveView.bottom-
...
camera.lookAt(camera.targetPosition);if(_that.bGlobalView)_that.render(scene,this.Gcamera,renderTarget,forceClear)};var lastBgView,lastBgyro;if(_that.bHidePanels){lastBgView=_that.bGlobalView;lastBgyro=_that.bGyroSimView}document.addEventListener("keydown",onDocumentKeyDown,false);function onDocumentKeyDown(event){var keyCode=event.which;switch(keyCode){case 27:case 71:_that.bHidePanels=!_that.bHidePanels;if(_that.bHidePanels){lastBgView=_that.bGlobalView;lastBgyro=_that.bGyroSimView;_that.bGlobalView=
false;_that.bGyroSimView=false}if(!_that.bHidePanels){_that.bGlobalView=lastBgView;_that.bGyroSimView=lastBgyro}break;case 82:_that.bRendering=!_that.bRendering;break}}};this.bHidePanels=false;this.bGyroSimViewInit=false;var _gyroView;var CGyroView=function(renderTarget,forceClear){this.init=function(){this.GyroSimCam=new THREE.PerspectiveCamera(90,_canvas.width/_canvas.height,0.01,1E4);this.GyroSimCam.position.x=0;this.GyroSimCam.position.y=0;this.GyroSimCam.position.z=20;this.GyroSimCam.up.x=0;
this.GyroSimCam.up.y=1;this.GyroSimCam.up.z=0;this.GyroSimScene=new THREE.Scene;this.GyroSimCam.lookAt(this.GyroSimScene.position);this.GyroSimScene.add(this.GyroSimCam);var boxScale=1;var boxX=10,boxY=2,boxZ=10;var geoBox=new THREE.BoxGeometry(boxX*boxScale,boxY*boxScale,boxZ*boxScale);var materBox=new THREE.MeshPhongMaterial({color:11184810,transparent:true,opacity:0.8,specular:15658751,shininess:20});this.GyroBox=new THREE.Mesh(geoBox,materBox);this.GyroSimScene.add(this.GyroBox);this.localSim=
new simulateGyro(this.GyroBox);var light=new THREE.DirectionalLight(16777215);light.position.set(0,5,5);this.GyroSimScene.add(light)};this.init();this.update=function(){var _left=Math.floor(_canvas.width*_that.GGyroSimView.left);var _bottom=Math.floor(_canvas.height*_that.GGyroSimView.bottom);var _width=Math.floor(_canvas.width*_that.GGyroSimView.width);var _height=Math.floor(_canvas.height*_that.GGyroSimView.height);_that.setViewport(_left,_bottom,_width,_height);_that.setScissor(_left,_bottom,_width,
_height);_that.enableScissorTest(true);_that.setClearColor((new THREE.Color).setRGB(0.11,0.12,0.18));this.GyroSimCam.aspect=_width/_height;this.GyroSimCam.updateProjectionMatrix();this.localSim.update();if(_that.bGyroSimView)_that.render(this.GyroSimScene,this.GyroSimCam,renderTarget,forceClear)}};var Leia_compute_renderViews=function(scene,camera,renderTarget,forceClear,shiftX,shiftY,_npart){var spanMode=_that._holoCamCenter.spanSphereMode;_that._holoCamCenter.eyeCenter.updateMatrix();var camPositionCenter=
...
4)){Leia_compute_renderViews(scene,camera,renderTarget,forceClear,0.5,0);Leia_compute_renderViews(scene,camera,renderTarget,forceClear,0,-0.5);Leia_compute_renderViews(scene,camera,renderTarget,forceClear,0.5,-0.5)}}else if(2==this._renderMode){Leia_compute_renderViews(scene,camera,this._shaderManager._swizzleRenderTarget,forceClear);if(this.nShaderMode==1||this.nShaderMode==2){Leia_compute_renderViews(scene,camera,this._shaderManager._swizzleRenderTargetSftX,forceClear,0.5,0);Leia_compute_renderViews(scene,
camera,this._shaderManager._swizzleRenderTargetSftY,forceClear,0,-0.5);Leia_compute_renderViews(scene,camera,this._shaderManager._swizzleRenderTargetSftXY,forceClear,0.5,-0.5)}this.setViewport(0,0,_canvas.width,_canvas.height);this.setScissor(0,0,_viewportWidth,_viewportHeight);this.enableScissorTest(true);if(this.nShaderMode==0)this._shaderManager.pass(this._shaderManager.basicShader,undefined);if(this.nShaderMode==1)this._shaderManager.pass(this._shaderManager.SSSShader,undefined);if(this.nShaderMode==
...
"tarPlane"){_this.object.scale.x+=delta*0.003;_this.object.scale.y+=delta*0.003;_this.object.scale.z+=delta*0.003}}var getMouseOnScreen=function(){var vector=new THREE.Vector2;return function(layerX,layerY){vector.set((layerX-_this.screen.left)/_this.screen.width,(layerY-_this.screen.top)/_this.screen.height);return vector}}();function intersectObjs(pointer,objs){var _MousePos=new THREE.Vector2;_MousePos.copy(getMouseOnScreen(pointer.layerX,pointer.layerY));pointerVec.set(_MousePos.x*2-1,-2*_MousePos.y+
1,0.5);projector.unprojectVector(pointerVec,camera);ray.set(camPos,pointerVec.sub(camPos).normalize());var intersections=ray.intersectObjects(objs,true);return intersections[0]?intersections[0]:false}};pickControls.prototype=Object.create(THREE.Object3D.prototype);
