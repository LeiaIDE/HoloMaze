...
var animate, initScene, render, _boxes = [],
  ...
  wall_yard_z2, wall_yard_z3, wall_yard_z4, wall_yard_z5, wall_yard_z6,
  wall_yard_z7, wall_yard_z8,
  wall_yard_x1, wall_yard_x2, wall_yard_x3, wall_yard_x4, wall_yard_x5,
  wall_yard_x6,
...
function initScene() {
  ...
  Physijs.scripts.ammo =
    'https://holodevuserresource.s3.amazonaws.com/ammo.js';
  ...
  var bumper_geom_z = new THREE.BoxGeometry(yard_z * 2 - 2.5 * wall_thick,
    bush, wall_thick);
  ...
  var bumper_geom_x_s = new THREE.BoxGeometry(wall_thick, bush *
    wall_strongScale, yard_z * 2);
  // x1
  //var material_x1 = Physijs.createMaterial(new THREE.MeshBasicMaterial({ color: 0x216000  }), .4, .8);
  ...
  // z1
  var bumper_geom_z_s = new THREE.BoxGeometry(yard_z * 2 - 2.5 * wall_thick,
    bush * wall_strongScale, wall_thick);
  ...
  // z2
  var slope_geo_z2 = new THREE.BoxGeometry(yard_z * 5.6 / 8, wall_thick,
    road_width);
  ...
  // door_1
  door_yard_1 = (wall_z7_x - wall_yard_z7 / 2) - (wall_z5_x + wall_yard_z5 /
    2);
  ...
  var door_1_x = ((wall_z7_x - wall_yard_z7 / 2) + (wall_z5_x + wall_yard_z5 /
    2)) / 2;
  ...
  groundMatrix.makeRotationFromEuler(new THREE.Euler(THREE.Math.degToRad(
    lastRoll), 0, THREE.Math.degToRad(lastPitch)));
...
function handleCollision(collided_with, linearVelocity, angularVelocity) {
  ...
  case 1:
    ...
    this.applyCentralImpulse(new THREE.Vector3(impulseForce.x, impulseForce.y,
      impulseForce.z));
    collided_with.applyCentralImpulse(new THREE.Vector3(-impulseForce.x *
      0.0000005, -impulseForce.y * 0.0000005, -impulseForce.z * 0.0000005
    ));
    break;
  case 2:
...
function render() {
  ...
  ground.matrix.makeRotationFromEuler(new THREE.Euler(totalRoll, 0,
    totalPitch));
  var quaternion = new THREE.Quaternion()
    .setFromEuler(new THREE.Euler(totalRoll, 0, totalPitch));
  var update = {
  ...
  bottom.matrix = r1Mat;
  var qqq = new THREE.Quaternion()
    .setFromRotationMatrix(r1Mat);
  var update_bottom = {
    id: bottom._physijs.id
  };
  var t1Vec = new THREE.Vector3()
    .setFromMatrixPosition(r1Mat);
  update_bottom.pos = {
    x: t1Vec.x,
    y: t1Vec.y,
    z: t1Vec.z
  ...
  rSelflogo.makeRotationFromEuler(new THREE.Euler(THREE.Math.degToRad(-90), 0,
    0));
  ...
  tMat.makeTranslation(-yard_x + wall_thick / 2, bush / 2 + ground_thick / 2,
    0);
  ...
  tMat2.makeTranslation(yard_x - wall_thick / 2, bush / 2 + ground_thick / 2,
    0);
  ...
  tMat3.makeTranslation(0, bush / 2 + ground_thick / 2, -yard_x + wall_thick /
    2);
  ...
  tMat4.makeTranslation(0, bush / 2 + ground_thick / 2, yard_x - wall_thick /
    2);
  ...
  var tVec4 = new THREE.Vector3()
    .setFromMatrixPosition(rMat4);
  update_bumper.pos = {
    x: tVec4.x,
    y: tVec4.y,
    z: tVec4.z
  ...
  tMat_s.makeTranslation(-yard_x - wall_thick / 2, bush / 2 + ground_thick /
    2, 0);
  ...
  var tVec_s = new THREE.Vector3()
    .setFromMatrixPosition(rMat_s);
  update_bumper_s.pos = {
    x: tVec_s.x,
    y: tVec_s.y,
    z: tVec_s.z
  ...
  tMat2_s.makeTranslation(yard_x + wall_thick / 2, bush / 2 + ground_thick /
    2, 0);
  ...
  var tVec2_s = new THREE.Vector3()
    .setFromMatrixPosition(rMat2_s);
  update_bumper_s.pos = {
    x: tVec2_s.x,
    y: tVec2_s.y,
    z: tVec2_s.z
  ...
  tMat3_s.makeTranslation(0, bush / 2 + ground_thick / 2, -yard_x -
    wall_thick / 2);
  ...
  var tVec3_s = new THREE.Vector3()
    .setFromMatrixPosition(rMat3_s);
  update_bumper_s.pos = {
    x: tVec3_s.x,
    y: tVec3_s.y,
    z: tVec3_s.z
  ...
  tMat4_s.makeTranslation(0, bush / 2 + ground_thick / 2, yard_x + wall_thick /
    2);
  ...
  var tVec4_s = new THREE.Vector3()
    .setFromMatrixPosition(rMat4_s);
  update_bumper_s.pos = {
    x: tVec4_s.x,
    y: tVec4_s.y,
    z: tVec4_s.z
  ...
  tMat_slopez1.makeTranslation(-yard_x + 2 * road_width, wall_thick / 2 +
    ground_thick / 2, -yard_z + road_width / 2);
  ...
  var tVec_slopez1 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_slopez1);
  update_slopez1.pos = {
    x: tVec_slopez1.x,
    y: tVec_slopez1.y,
    z: tVec_slopez1.z
  };
  var quaternion_slope = new THREE.Quaternion()
    .setFromRotationMatrix(rMat_slopez1);
  update_slopez1.quat = {
  ...
  var tVec_slopez2 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_slopez2);
  update_slopez2.pos = {
    x: tVec_slopez2.x,
    y: tVec_slopez2.y,
    z: tVec_slopez2.z
  };
  var quaternion_slope_z2 = new THREE.Quaternion()
    .setFromRotationMatrix(rMat_slopez2);
  update_slopez2.quat = {
  ...
  var tVec_wallz1 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallz1);
  update_bumper.pos = {
    x: tVec_wallz1.x,
    y: tVec_wallz1.y,
    z: tVec_wallz1.z
  ...
  var tVec_wallz2 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallz2);
  update_bumper.pos = {
    x: tVec_wallz2.x,
    y: tVec_wallz2.y,
    z: tVec_wallz2.z
  ...
  var tVec_wallx1 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallx1);
  update_bumper.pos = {
    x: tVec_wallx1.x,
    y: tVec_wallx1.y,
    z: tVec_wallx1.z
  ...
  var tVec_wallx2 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallx2);
  update_bumper.pos = {
    x: tVec_wallx2.x,
    y: tVec_wallx2.y,
    z: tVec_wallx2.z
  ...
  var tVec_wallz3 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallz3);
  update_bumper.pos = {
    x: tVec_wallz3.x,
    y: tVec_wallz3.y,
    z: tVec_wallz3.z
  ...
  var tVec_wallx3 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallx3);
  update_bumper.pos = {
    x: tVec_wallx3.x,
    y: tVec_wallx3.y,
    z: tVec_wallx3.z
  ...
  var tVec_wallz4 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallz4);
  update_bumper.pos = {
    x: tVec_wallz4.x,
    y: tVec_wallz4.y,
    z: tVec_wallz4.z
  ...
  var tVec_wallx4 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallx4);
  update_bumper.pos = {
    x: tVec_wallx4.x,
    y: tVec_wallx4.y,
    z: tVec_wallx4.z
  ...
  var tVec_wallz5 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallz5);
  update_bumper.pos = {
    x: tVec_wallz5.x,
    y: tVec_wallz5.y,
    z: tVec_wallz5.z
  ...
  var tVec_wallx5 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallx5);
  update_bumper.pos = {
    x: tVec_wallx5.x,
    y: tVec_wallx5.y,
    z: tVec_wallx5.z
  ...
  var tVec_wallz6 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallz6);
  update_bumper.pos = {
    x: tVec_wallz6.x,
    y: tVec_wallz6.y,
    z: tVec_wallz6.z
  ...
  var tVec_wallx6 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallx6);
  update_bumper.pos = {
    x: tVec_wallx6.x,
    y: tVec_wallx6.y,
    z: tVec_wallx6.z
  ...
  var tVec_wallz7 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallz7);
  update_bumper.pos = {
    x: tVec_wallz7.x,
    y: tVec_wallz7.y,
    z: tVec_wallz7.z
  ...
  var tVec_wallz8 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_wallz8);
  update_bumper.pos = {
    x: tVec_wallz8.x,
    y: tVec_wallz8.y,
    z: tVec_wallz8.z
  ...
  var tVec_tar_1 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_tar_1);
  update_bumper.pos = {
    x: tVec_tar_1.x,
    y: tVec_tar_1.y,
    z: tVec_tar_1.z
  ...
  var tVec_tar_2 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_tar_2);
  update_bumper.pos = {
    x: tVec_tar_2.x,
    y: tVec_tar_2.y,
    z: tVec_tar_2.z
  ...
  var tVec_tar_3 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_tar_3);
  update_bumper.pos = {
    x: tVec_tar_3.x,
    y: tVec_tar_3.y,
    z: tVec_tar_3.z
  ...
  var tVec_door1 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_door1);
  update_bumper.pos = {
    x: tVec_door1.x,
    y: tVec_door1.y,
    z: tVec_door1.z
  ...
  var tVec_door2 = new THREE.Vector3()
    .setFromMatrixPosition(rMat_door2);
  update_bumper.pos = {
    x: tVec_door2.x,
    y: tVec_door2.y,
    z: tVec_door2.z
  ...
  if (undefined === scene.getObjectByName("ball")) spawnBall(_gP.x, _gP.y,
    _gP.z);
  ...
  if (deltaRoll > 2 || deltaRoll < -2 || deltaPitch > 2 || deltaPitch < -2) {
    ...
    boxMat_last.makeRotationFromEuler(new THREE.Euler(THREE.Math.degToRad(
      lastRoll), 0, THREE.Math.degToRad(lastPitch)));
...
function worker() {
  ...
  createShape = function (description) {
    ...
    case 'plane':
      cache_key = 'plane_' + description.normal.x + '_' + description.normal
        .y + '_' + description.normal.z;
    ...
    case 'box':
      cache_key = 'box_' + description.width + '_' + description.height +
        '_' + description.depth;
      if ((shape = getShapeFromCache(cache_key)) === null) {
    ...
    case 'cylinder':
      cache_key = 'cylinder_' + description.width + '_' + description.height +
        '_' + description.depth;
      if ((shape = getShapeFromCache(cache_key)) === null) {
        _vec3_1.setX(description.width / 2);
        _vec3_1.setY(description.height / 2);
    ...
    case 'capsule':
      ...
      if ((shape = getShapeFromCache(cache_key)) === null) {
        ...
        shape = new Ammo.btCapsuleShape(description.radius, description.height -
          2 * description.radius);
        setShapeCache(cache_key, shape);
      }
      break;
  ...
  public_functions.init = function (params) {
    ...
    if (SUPPORT_TRANSFERABLE) {
      ...
      worldreport = new Float32Array(2 + REPORT_CHUNKSIZE *
        WORLDREPORT_ITEMSIZE); // message id + # of objects to report + chunk size * # of values per object
      collisionreport = new Float32Array(2 + REPORT_CHUNKSIZE *
        COLLISIONREPORT_ITEMSIZE); // message id + # of collisions to report + chunk size * # of values per object
      vehiclereport = new Float32Array(2 + REPORT_CHUNKSIZE *
        VEHICLEREPORT_ITEMSIZE); // message id + # of vehicles to report + chunk size * # of values per object
      constraintreport = new Float32Array(2 + REPORT_CHUNKSIZE *
        CONSTRAINTREPORT_ITEMSIZE); // message id + # of constraints to report + chunk size * # of values per object
    } else {
    ...
    world = new Ammo.btDiscreteDynamicsWorld(dispatcher, broadphase, solver,
  ...
  public_functions.addObject = function (description) {
    ...
    rbInfo = new Ammo.btRigidBodyConstructionInfo(description.mass,
      motionState, shape, _vec3_1);
  ...
  public_functions.addVehicle = function (description) {
    ...
    vehicle = new Ammo.btRaycastVehicle(vehicle_tuning, _objects[
      description.rigidBody], new Ammo.btDefaultVehicleRaycaster(world));
  ...
  public_functions.addWheel = function (description) {
    ...
    if (SUPPORT_TRANSFERABLE) {
      vehiclereport = new Float32Array(1 + _num_wheels *
        VEHICLEREPORT_ITEMSIZE); // message id & ( # of objects to report * # of values per object )
      vehiclereport[0] = MESSAGE_TYPES.VEHICLEREPORT;
    } else {
  ...
  public_functions.removeObject = function (details) {
    ...
    if (_compound_shapes[details.id]) Ammo.destroy(_compound_shapes[details
      .id]);
    if (_noncached_shapes[details.id]) Ammo.destroy(_noncached_shapes[
      details.id]);
    var ptr = _objects[details.id].a !== undefined ? _objects[details.id].a :
  ...
  public_functions.updateTransform = function (details) {
    ...
    _object.getMotionState()
      .getWorldTransform(_transform);
  ...
  public_functions.addConstraint = function (details) {
    ...
    case 'conetwist':
      ...
      rotation = transforma.getRotation();
      rotation.setEulerZYX(-details.axisa.z, -details.axisa.y, -details.axisa
        .x);
      transforma.setRotation(rotation);

      rotation = transformb.getRotation();
      rotation.setEulerZYX(-details.axisb.z, -details.axisb.y, -details.axisb
        .x);
      transformb.setRotation(rotation);
    ...
    if (SUPPORT_TRANSFERABLE) {
      constraintreport = new Float32Array(1 + _num_constraints *
        CONSTRAINTREPORT_ITEMSIZE); // message id & ( # of objects to report * # of values per object )
      constraintreport[0] = MESSAGE_TYPES.CONSTRAINTREPORT;
    } else {
    ...
    if (world) {
      ...
      if (!params.timeStep) {
        if (last_simulation_time) {
          params.timeStep = 0;
          while (params.timeStep + last_simulation_duration <=
            fixedTimeStep) {
      ...
      params.maxSubSteps = params.maxSubSteps || Math.ceil(params.timeStep /
        fixedTimeStep); // If maxSubSteps is not defined, keep the simulation fully up to date

      last_simulation_duration = Date.now();
      world.stepSimulation(params.timeStep, params.maxSubSteps,
        fixedTimeStep);
      ...
      last_simulation_duration = (Date.now() - last_simulation_duration) /
        1000;
  ...
  public_functions.hinge_setLimits = function (params) {
    _constraints[params.constraint].setLimit(params.low, params.high, 0,
      params.bias_factor, params.relaxation_factor);
  };
    ...
    for (index in _vehicles) { //noprotect
      if (_vehicles.hasOwnProperty(index)) {
        ...
        for (j = 0; j < vehicle.getNumWheels(); j++) { //noprotect
          ...
          transform = vehicle.getWheelInfo(j)
  ...
  reportConstraints = function () {
    ...
    if (SUPPORT_TRANSFERABLE) {
      if (constraintreport.length < 2 + _num_constraints *
        CONSTRAINTREPORT_ITEMSIZE) {
        constraintreport = new Float32Array(
          2 + // message id & # objects in report
          (Math.ceil(_num_constraints / REPORT_CHUNKSIZE) *
            REPORT_CHUNKSIZE) * CONSTRAINTREPORT_ITEMSIZE // # of values needed * item size
        );
        constraintreport[0] = MESSAGE_TYPES.CONSTRAINTREPORT;
      }
    }
